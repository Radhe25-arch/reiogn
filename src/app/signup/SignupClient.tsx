...
export default function SignupPage() {
  ...
  // Fetch paid user count
  useEffect(() => {
    fetch('/api/stats/paid-users')
      .then(r => r.json())
      .then(d => { if (d.success) setPaidCount(d.data.count) })
      .catch(() => setPaidCount(847))
  }, [])

  // Animate counter from 0 to target
  useEffect(() => {
    if (paidCount === null) return
    let frame = 0
    const total = 60
    const target = paidCount
    const iv = setInterval(() => {
      frame++
      const progress = frame / total
      const ease = 1 - Math.pow(1 - progress, 3)
      setCountAnim(Math.floor(ease * target))
  ...
  useEffect(() => {
    ...
    for(let i=0;i<110;i++) particles.push({
      ...
      size:Math.random()*2+.4, alpha:Math.random()*.5+.1,
      color:Math.random()>.5?'#b8ff00':'#00f0c8',
    })
    const shapes:{x:number;y:number;rot:number;vr:number;vy:number;sides:number;size:number;alpha:number}[]=[]
    for(let i=0;i<12;i++) shapes.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,rot:Math.random()*Math.PI*2,vr:(Math.random()-.5)*.004,vy:-Math.random()*.15-.05,sides:[3,4,6,8][Math.floor(Math.random()*4)],size:Math.random()*40+18,alpha:Math.random()*.07+.02})
    function draw(){
      canvas!.width=window.innerWidth; canvas!.height=window.innerHeight
      ...
      shapes.forEach(s=>{
        ...
        c.save();c.beginPath()
        for(let i=0;i<s.sides;i++){const a=(i/s.sides)*Math.PI*2+s.rot;const px=s.x+Math.cos(a)*s.size;const py=s.y+Math.sin(a)*s.size;i===0?c.moveTo(px,py):c.lineTo(px,py)}
        c.closePath();c.strokeStyle=`rgba(184,255,0,${s.alpha})`;c.lineWidth=1;c.stroke();c.restore()
      })
      particles.forEach(p=>{
        p.x+=p.vx;p.y+=p.vy
      ...
      for(let i=0;i<particles.length;i++) for(let j=i+1;j<particles.length;j++){
        ...
        if(d<100){c.beginPath();c.moveTo(particles[i].x,particles[i].y);c.lineTo(particles[j].x,particles[j].y);c.strokeStyle=`rgba(184,255,0,${.06*(1-d/100)})`;c.lineWidth=.5;c.stroke()}
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res  = await fetch('/api/auth/signup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email,password,referralCode:referral||undefined})})
      ...
      setError(isOffline?'You appear to be offline.':'Unable to reach the server. Please make sure the app is running.')
      setLoading(false)
    }
  }
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        :root{--bg:#07080f;--s1:#0c0e1a;--s2:#111320;--lime:#b8ff00;--teal:#00f0c8;--rose:#ff4f72;--bd:rgba(255,255,255,0.06);--bd2:rgba(255,255,255,0.11);--t1:#eef0ff;--t2:rgba(238,240,255,0.55);--t3:rgba(238,240,255,0.28);--t4:rgba(238,240,255,0.10);}
        html,body{min-height:100vh;background:var(--bg);color:var(--t1);font-family:'Inter',sans-serif;overflow-x:hidden;}
        .bg-canvas{position:fixed;inset:0;z-index:0;pointer-events:none;}
        .bg-grad{position:fixed;inset:0;z-index:0;pointer-events:none;background:radial-gradient(ellipse 60% 50% at 20% 50%,rgba(184,255,0,0.04),transparent),radial-gradient(ellipse 50% 60% at 80% 30%,rgba(0,240,200,0.04),transparent);}
        .page{position:relative;z-index:1;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:80px 20px 40px;}
        ...
        /* Social proof bar */
        .social-bar{background:rgba(184,255,0,0.05);border:1px solid rgba(184,255,0,0.15);border-radius:10px;padding:12px 18px;margin-bottom:16px;display:flex;align-items:center;gap:12px;width:100%;max-width:420px;}
        .sb-avatars{display:flex;}
        .sb-av{width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,#b8ff00,#00f0c8);display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:8px;font-weight:700;color:#07080f;border:1.5px solid #07080f;margin-left:-6px;flex-shrink:0;}
        .sb-av:first-child{margin-left:0;}
        .sb-text{font-size:11.5px;color:rgba(255,255,255,0.7);line-height:1.4;flex:1;}
        .sb-count{font-family:'Syne',sans-serif;font-size:13px;font-weight:800;color:#b8ff00;}
        .sb-live{width:6px;height:6px;border-radius:50%;background:#b8ff00;animation:blink 1.8s infinite;flex-shrink:0;}
        @keyframes blink{0%,100%{opacity:1}50%{opacity:.15}}
        /* Card */
        .card{background:rgba(12,14,26,0.88);backdrop-filter:blur(24px);border:1px solid var(--bd2);border-radius:16px;padding:38px 40px;width:100%;max-width:420px;box-shadow:0 0 0 1px rgba(184,255,0,0.04),0 32px 80px rgba(0,0,0,0.7);}
        @media(max-width:480px){.card{padding:26px 20px;border-radius:12px;}}
        ...
        .terms{text-align:center;margin-top:14px;font-size:11px;color:var(--t4);line-height:1.6;}
        .terms a{color:rgba(184,255,0,0.45);text-decoration:none;}
        /* Success state */
        .success-card{text-align:center;padding:48px 32px;}
        .success-icon{font-size:52px;margin-bottom:20px;display:block;animation:popIn .5s cubic-bezier(.175,.885,.32,1.275);}
        @keyframes popIn{from{transform:scale(0);opacity:0}to{transform:scale(1);opacity:1}}
        .success-h{font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:#fff;margin-bottom:8px;}
        .success-sub{font-size:13px;color:var(--t2);line-height:1.6;margin-bottom:24px;}
        .success-bar{background:rgba(184,255,0,0.08);border:1px solid rgba(184,255,0,0.2);border-radius:8px;padding:14px 18px;margin-top:16px;}
        .success-stat{font-family:'Syne',sans-serif;font-size:28px;font-weight:800;color:#b8ff00;display:block;}
        .success-label{font-family:'JetBrains Mono',monospace;font-size:9px;color:rgba(255,255,255,0.35);letter-spacing:1.5px;text-transform:uppercase;margin-top:4px;display:block;}
        .paid-ticker{display:flex;align-items:center;gap:8px;justify-content:center;margin-bottom:8px;}
        .ticker-dot{width:7px;height:7px;border-radius:50%;background:#b8ff00;animation:blink 1.2s infinite;}
        .ticker-txt{font-family:'JetBrains Mono',monospace;font-size:10px;color:rgba(255,255,255,0.5);letter-spacing:1px;text-transform:uppercase;}
      `}</style>

      {mounted && <canvas ref={canvasRef} className="bg-canvas" />}
      ...
      <nav>
        ...
        <a href="/login" style={{fontSize:'13px',color:'var(--t2)',textDecoration:'none'}}>Sign in →</a>
      </nav>
      ...
      <div className="page">
        ...
        <div className="social-bar">
          <div className="sb-avatars">
            {['AK','MR','SV','JP','KL'].map((av,i)=>(
              <div key={i} className="sb-av">{av}</div>
            ))}
          </div>
          <div className="sb-text">
            <span className="sb-count">{paidCount !== null ? countAnim.toLocaleString() : '...'}</span>
            {' '}people on a paid plan right now
          </div>
        ...
        <div className="card">
          {success ? (
            <div className="success-card">
              ...
              <p className="success-sub">Welcome to REIOGN. You've got 500 tokens and 3 days free.<br />Taking you to the dashboard…</p>
              <div className="success-bar">
                <div className="paid-ticker">
                  <div className="ticker-dot" />
                  <span className="ticker-txt">Live paid users</span>
                </div>
                <span className="success-stat">{paidCount !== null ? countAnim.toLocaleString() : '—'}</span>
                <span className="success-label">people on paid plans — join them</span>
              </div>
            </div>
          ) : (
            <>
              <div className="badge"><div className="badge-dot" />Create Account</div>
              <h1>Join <span style={{color:'var(--lime)'}}>REIOGN</span></h1>
              <p className="sub">Start your 3-day free trial. No card required.</p>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="field">
                  ...
                  <div className="inp-wrap">
                    <input type={showPass?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 8 characters" required autoComplete="new-password" style={{paddingRight:'48px'}}/>
                    <button type="button" className="pass-toggle" onClick={()=>setShowPass(v=>!v)}>{showPass?'hide':'show'}</button>
                  </div>
                </div>
                <div className="field">
                  <label>Referral code <span style={{color:'var(--t4)',fontWeight:400}}>(optional)</span></label>
                  <input value={referral} onChange={e=>setReferral(e.target.value.toUpperCase())} placeholder="REIOGN-XXXX" autoComplete="off"/>
                </div>
              ...
              <p className="footer-txt">Already have an account? <a href="/login">Sign in</a></p>
